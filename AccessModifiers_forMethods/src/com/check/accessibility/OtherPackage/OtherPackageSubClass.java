...
public class OtherPackageSubClass extends SamePackageSameClass{
...
=>public Static methods are accessible through class name where as public Instance methods are accessible through object of the class.
 =>protected Static methods are accessible through class name where as protected Instance variables are accessible directly.
 =>but default(static/non-static) are not accessible in other package even though by subclass that is because of the reason "Encapsulation at package level".and private(static/non-static) are not accessible  in other package even though by subclass that is because of the reason "Encapsulation at class level".
 */
	}
}
